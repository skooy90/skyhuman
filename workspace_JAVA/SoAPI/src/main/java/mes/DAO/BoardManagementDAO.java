...
public class BoardManagementDAO {
	...
	public List<BoardDTO> selectLatestNotices(int limit) {
		List<BoardDTO> list = new ArrayList<>();
		String sql = "SELECT * FROM (" + "  SELECT POST_NO, BO_TITLE, BO_CATEGORY, BO_CREATION_DATE" + "    FROM BOARD"
				+ "   WHERE BO_CATEGORY = '공지사항'" + "   ORDER BY BO_CREATION_DATE DESC" + ") WHERE ROWNUM <= ?";
		...
		try {
			...
			while (rs.next()) {
				...
				d.setPostNo(rs.getString("POST_NO"));
				d.setBoTitle(rs.getString("BO_TITLE"));
				d.setBoCategory(rs.getString("BO_CATEGORY"));
				d.setBoCreationDate(rs.getDate("BO_CREATION_DATE"));
				list.add(d);
			}
		} catch (Exception e) {
			e.printStackTrace();
	...
	private static final BoardManagementDAO INSTANCE = new BoardManagementDAO();

	public static BoardManagementDAO getInstance() {
		return INSTANCE;
	}
	...
	public static class BoardStatistics {
		...
		public void setTotalBoards(int totalBoards) {
			this.totalBoards = totalBoards;
		}

		public int getTodayBoards() {
			return todayBoards;
		}

		public void setTodayBoards(int todayBoards) {
		...
		public void setCategoryCounts(java.util.Map<String, Integer> categoryCounts) {
